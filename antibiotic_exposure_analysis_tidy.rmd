...
## Tank water Physchem
```{r}
# What is distribution of water chemistry between tanks
tank <- read_csv("data/tank.water.information.csv", col_types = c("icfidddddddddddfc"), col_select = -SN) %>%
  filter(Date != "01/06/2023") %>%
  mutate(Date = as.Date(Date, format = "%d/%m/%Y")) %>%
  rowwise() %>%
  mutate(Avg.temp.C = mean(c_across(c(starts_with("Temp.C."))), na.rm = T))

tanks.filt <- tank %>%
  select(-Days.post.exposure.start, -Temp.C.pH.meter, -Temp.C.cond.meter,
         -Temp.C.dO2.meter, -Person.responsible, -Notes)

(dist.water.chem.p <- tanks.filt %>%
  pivot_longer(-c("Date", "Tank"), names_to = "metric", values_to = "value") %>%
...
tanks.filt %>%
  tibble() %>%
  select(-Date) %>%
  summarise(across(where(is.double), list(sd = sd))) %>%
...
## Carp growth rates
```{r}
carp <- read_csv("data/Carp_PIT.tag_weight_length_tank.no.UPDATED.csv") %>%
  unite(tank, treatment, replicate, remove = F, sep = "") %>%
  mutate(treatment = factor(treatment, levels = c("Control", "Low", "High")))

weight.carp <- carp %>%
  ggplot(aes(x = date, y = weight.g,
    group = PIT.tag.num, colour = treatment)) +
    geom_point(size = 1) +
    geom_line() +
    facet_wrap(~ treatment, ncol = 3) +
  scale_fill_manual(values = clrs.tanks, aesthetics = c("fill", "colour")) +
  theme_bw()

length.carp <- carp %>%
  ggplot(aes(x = date, y = fork.length.cm,
    group = PIT.tag.num, colour = treatment)) +
    geom_point(size = 1) +
    geom_line() +
    facet_wrap(~ treatment, ncol = 3) +
...
length.carp + weight.carp +
  patchwork::plot_annotation(tag_levels = 'A') +
  plot_layout(guides = "collect")

LW3 <- carp %>%
  group_by(PIT.tag.num) %>%
  mutate(daily.length.gain.cm = diff(fork.length.cm)/as.numeric(diff(date)),
         daily.weight.gain.g = diff(weight.g)/as.numeric(diff(date)))

weight.lmer <- lmerTest::lmer(weight.g ~ treatment + (1 + treatment | treatment / tank), data = carp) %>%
  broom.mixed::tidy()

length.lmer <- lmerTest::lmer(fork.length.cm ~ treatment + (1 + treatment | treatment / tank), data = carp) %>%
  broom.mixed::tidy()

LW3 %>%
  group_by(treatment) %>%
  select(-c(date, PIT.tag.num, replicate, tank)) %>%
...
# Figure 7-10, Figure 6 below this block
...
# Table 6, Figure 11
# How different are individual carp microbiomes
```{r}
# How different are the same samples over time
ps <- readRDS("data/combined_ASV_PS.rds")
metadata <- samdat_tbl(ps) %>%
  rename(Sample = .sample_name)
  
clrs.fish <- clrs.tanks[levels(metadata$tank)] %>%
  data.frame() %>%
  rownames_to_column(var = "tank") %>%
  rename(colour = ".")

fish.filt <- metadata %>%
  distinct(tank, PIT.tag.num) %>%
  filter(!is.na(PIT.tag.num)) %>%
  left_join(clrs.fish, by = "tank")

clrs.fish <- setNames(fish.filt$colour, fish.filt$PIT.tag.num)

clrs <- c(clrs.tanks, clrs.fish)

df <- read_csv("data/alpha.diversity.mean.csv") %>%
  left_join(metadata, by = "Sample") %>%
  mutate(tank = as.factor(paste0(treatment, replicate))) %>%
  pivot_longer(cols = c("Chao1", "Shannon", "PD"), names_to = "alpha.div.name", values_to = "alpha.div.value") %>%
  mutate(tank = paste0(treatment, replicate)) %>%
  filter(sample.type == "Filter") %>%
  group_by(tank, Days.post.exposure.start, alpha.div.name) %>%
  mutate(minimum = min(alpha.div.value),
         maximum = max(alpha.div.value),
         average = mean(alpha.div.value),
         sd = sd(alpha.div.value)) %>%
  ungroup()

# Figure 11
ggplot(df, aes(x = Days.post.exposure.start, y = alpha.div.value)) +
  annotate("rect", xmin = 0, xmax = 14, ymin = -Inf, ymax = Inf, 
           alpha=0.2, fill="black") +
  #geom_ribbon(aes(ymin = minimum, ymax = maximum, fill = tank), alpha = 0.25) +
  geom_ribbon(aes(ymin = average - sd, ymax = average + sd, fill = tank), alpha = 0.25) +
  #geom_boxplot(aes(x = as.numeric(levels(Days.post.exposure.start)))) +
  geom_line(aes(y = average, group = tank, colour = tank), linewidth = 2) +
  geom_line(aes(group = PIT.tag.num, color = PIT.tag.num), alpha = 0.1, linewidth = 1) +
  facet_wrap(alpha.div.name~treatment, scale = "free_y") +
  scale_colour_manual(values = clrs, aesthetics = c("fill", "colour")) +
  theme_bw() +
  xlab("Days post exposure start") +
  ylab("Alpha diversity") +
...
df <- alpha.div %>%
  filter(sample.type == "Swab") %>%
  rename(TP = Days.post.exposure.start) %>%
  select(PIT.tag.num, Chao1, Shannon, PD, TP, treatment, tank) %>%
  arrange(PIT.tag.num, TP) %>%
  mutate(TP = factor(TP, levels = sort(unique(.$TP))))

df %>%
  group_by(PIT.tag.num) %>%
  mutate(Chao1.diff = c(diff(Chao1), NA),
         Shannon.diff = c(diff(Shannon), NA),
         PD.diff = c(diff(PD), NA)) %>%
  filter(!is.na(PD.diff)) %>%
  lmerTest::lmer(PD.diff ~ treatment*TP + (1 + treatment|treatment/tank/PIT.tag.num), data = .) %>%
  broom.mixed::tidy() %>%
  filter(str_detect(term, ":")) %>%
  mutate(`B.H. p adjusted` = p.adjust(p.value, method = "BH"),
         `Differences between time points` = 
           str_replace(term, ".*TP7", "7 to 14"),
         `Differences between time points` = 
           str_replace(`Differences between time points`, ".*TP14", "14 to 21"),
         `Differences between time points` = 
           str_replace(`Differences between time points`, ".*TP21", "21 to 28"),
         term = str_replace(term, ".*High.*", "High"),
         term = str_replace(term, ".*Low.*", "Low"),
         `Alpha diversity metric` = "PD") %>%
  rename(Treatment = term) %>%
  select(`Alpha diversity metric`, Treatment, `Differences between time points`, estimate, std.error, statistic, df, `B.H. p adjusted`)

df %>%
  group_by(treatment, TP, tank) %>%
  mutate(Chao1.trt.TP.tk.mean.diff = Chao1/mean(Chao1)) %>%
  ungroup(tank) %>%
  mutate(Chao1.trt.TP.mean.diff = Chao1/mean(Chao1)) %>%
  ungroup(TP) %>%
  mutate(Chao1.trt.mean.diff = Chao1/mean(Chao1)) %>%
  view()
```
...
metadata <- samdat_tbl(ps) %>%
  select(.sample_name, tank, treatment, PIT.tag.num, Days.post.exposure.start)
...
PCA.swab <- rda(otu_table(ps.swab), scale = F)
PCA.filt <- rda(otu_table(ps.filt), scale = F)

CAPTION <- paste0(nrow(samdat_tbl(ps.swab)), " samples & ", length(taxa_names(ps)), " taxa (unique). PCA , tax_transformation = None, dist = None")

scores(PCA.swab)$sites %>%
  data.frame() %>%
  rownames_to_column(var = "Sample") %>%
  left_join(metadata, by = join_by(Sample == .sample_name)) %>%
  group_by(tank, Days.post.exposure.start) %>%
  mutate(average = mean(PC1),
         stdev = sd(PC1)) %>%
  ungroup() %>%
  arrange(Days.post.exposure.start, PIT.tag.num) %>%
  ggplot(aes(x = PC1, y = PC2)) +
  geom_path(aes(group = PIT.tag.num),  colour = "grey70", linewidth = 1) +
  geom_point(aes(colour = as.factor(Days.post.exposure.start)), size = 3) +
  theme_bw() +
  facet_wrap(~tank) +
  #scale_colour_manual(values = clrs)
  scale_colour_brewer(palette = "Set1") +
  guides(color=guide_legend("Days post exposure start")) +
  theme(legend.position = "bottom") +
  labs(caption = CAPTION) +
  xlab(paste0("PC1 (", round(summary(PCA.swab)$cont$importance["Proportion Explained", "PC1"]*100, digits = 2), " %)")) +
  ylab(paste0("PC2 (", round(summary(PCA.swab)$cont$importance["Proportion Explained", "PC2"]*100, digits = 2), " %)"))
