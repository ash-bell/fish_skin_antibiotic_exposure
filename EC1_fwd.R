...
errF_1 <- try(learnErrors(filtFs, multithread = T, nbases = 1e10,
                      errorEstimationFunction = loessErrfun_mod1, 
                      verbose = T))
saveRDS(errF_1, paste0(path,"/errF_1.rds"))

errF_1.p <- plotErrors(errF_1, nominalQ = T)
ggsave(paste0(path, "/errF_1.svg"), plot = errF_1.p, 
